...
from .codex_auth import build_codex_login_command, run_codex_command
...
def build_openai_responses_model(
    *,
    ...
    login_required_error: CodexOAuthLoginRequiredError | None = None
    try:
        credentials = resolve_codex_oauth_credentials()
    except CodexOAuthLoginRequiredError as error:
    ...
    if login_required_error is not None:
        raise login_required_error

    login_command = build_codex_login_command()
    raise CodexOAuthLoginRequiredError(
        "No Codex OAuth credentials found and no OPENAI_API_KEY fallback is configured. "
        f"Run `{' '.join(login_command)}` and retry.",
...
def _refresh_auth_file(*, auth_path: Path, auth_doc: dict[str, Any]) -> dict[str, Any]:
    ...
    auth_path.write_text(f"{json.dumps(refreshed_doc)}\n", encoding="utf-8")
