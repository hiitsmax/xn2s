...
> Historical context: this ExecPlan captures the earlier implementation of `report latest`. The current active report surface is now `report issues`, `report html`, `report latest`, and `report schedule`.
